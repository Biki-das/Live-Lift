...
class Car {
  ...
  carCenterX() {
    return (
      this.carLeftMargin + (this.carNumber - 1) * this.carHorizontalSpacing
    );
  }
  drawDoors() {
    const p = this.p;
...
class Controls {
  ...
  createKnobs(passengerLoadTypes) {
    ...
    ["None", "English"].forEach((p) => speakers.option(p));
    speakers.parent("#speakersParent");
    speakers.changed(
      () => (settings.speakersType = speakers.elt.selectedIndex)
    );
  }
}
...
class Dispatcher {
  ...
  processRiders() {
    ...
    this.riders = this.riders.filter(
      (rider) => rider.state !== RiderState.Exited
    );
...
class Rider {
  constructor(p, settings, startFloor, destFloor, dispatcher, stats, talker) {
    ...
    this.headColor = [p.random(255), p.random(255), p.random(255)];
    this.torsoColor = [p.random(255), p.random(255), p.random(255)];
    this.legColor = [p.random(255), p.random(255), p.random(255)];
  ...
  draw() {
    ...
    const torsoHeight = s(this.height / 2);
    const headSize = s(this.height / 8);
    const armLength = s(this.height / 3);
    const limbThickness = s(this.width / 4);
    const torsoWidth = s(this.width);
    const headHeight = legLength + torsoHeight + headSize / 2;
    p.pushed(() => {
      p.translate(this.pos.x, this.pos.y, this.pos.z);
      // Draw legs with unique color
      p.pushed(() => {
        p.translate(-torsoWidth / 4, legLength / 2, 0);
        p.fill(this.legColor[0], this.legColor[1], this.legColor[2]); // Unique color for legs
        p.cylinder(limbThickness, legLength);
      });
      p.pushed(() => {
        p.translate(torsoWidth / 4, legLength / 2, 0);
        p.cylinder(limbThickness, legLength);
      });
      // Draw torso with unique color
      p.pushed(() => {
        p.translate(0, legLength + torsoHeight / 2, 0);
        p.fill(this.torsoColor[0], this.torsoColor[1], this.torsoColor[2]); // Unique color for torso
        p.box(torsoWidth, torsoHeight, torsoWidth / 2);
      });
      // Draw arms with the same color as the torso
      p.pushed(() => {
        p.translate(-torsoWidth / 2, legLength + torsoHeight / 2, 0);
        p.fill(this.torsoColor[0], this.torsoColor[1], this.torsoColor[2]); // Same color as torso
        p.cylinder(limbThickness, armLength);
      });
      p.pushed(() => {
        p.translate(torsoWidth / 2, legLength + torsoHeight / 2, 0);
        p.cylinder(limbThickness, armLength);
      });
      // Draw head with unique color
      p.pushed(() => {
        p.translate(0, headHeight, 0);
        p.fill(this.headColor[0], this.headColor[1], this.headColor[2]); // Unique color for head
        p.sphere(headSize / 2);
      });
    });
  }
...
new p5((p) => {
  ...
  let ready = false;
  p.preload = function () {
    p.dingSound = p.loadSound("public/ding.wav");
  };
  p.setup = function () {
    ...
    talker.whenLoaded(() => {
      cars = Array.from(
        Array(settings.numCars).keys(),
        (n) => new Car(p, settings, stats, n + 1)
      );
  ...
  function setCanvasSize() {
    const m = $("#main");
    settings.geom.canvas = p.createVector(
      m.width() * 0.95,
      p.windowHeight * 0.92
  ...
  function showRiderStats() {
    ...
    const now = p.millis() / 1000;
    const waitingRiders = dispatcher.riders.filter(
      (r) => r.state === RiderState.Waiting
    );
    const waitSecs = waitingRiders.reduce(
      (accum, rider) => now - rider.arrivalTime + accum,
      0
    );
    const wait = s.waiting ? ` (${l(Math.round(waitSecs))} secs)` : "";
    const profit = s.payments - stats.costs.operating;
    $("#score").html(
      l(Math.round(Math.max(0, profit / (p.millis() / 1000 / 60))))
    );
  ...
  function setUpCamera() {
    ...
    if (settings.projectionType === 1) {
      ...
      if (settings.controlMode === 0 /* Auto */) {
        const avgCarY =
          cars.map((car) => car.y).reduce((a, b) => a + b, 0) / cars.length;
        p.camera(
          0,
          -avgCarY,
          p.height / 2.0 / p.tan((p.PI * 30.0) / 180.0),
